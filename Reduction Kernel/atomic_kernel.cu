...
__global__ void atomic_reduction_kernel(float* d_out, float* d_in, unsigned int size)
{
...
void atomic_reduction(float* d_out, float* d_in, int n_threads, unsigned int size)
{
    ...
    cudaOccupancyMaxActiveBlocksPerMultiprocessor(&num_blocks_per_sm, atomic_reduction_kernel, n_threads, n_threads * sizeof(float));

    cudaMemcpy(d_out, d_in, size * sizeof(float), cudaMemcpyDeviceToDevice);

    int n_blocks = min(num_blocks_per_sm * num_sms, (size + n_threads - 1) / n_threads);

    atomic_reduction_kernel << <n_blocks, n_threads, n_threads * sizeof(float), 0 >> > (d_out, d_out, size);
}