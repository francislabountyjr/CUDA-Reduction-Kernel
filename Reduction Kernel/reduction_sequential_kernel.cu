...
__global__ void sequential_reduction_kernel(float* d_out, float* d_in, unsigned int size)
{
...
void sequential_reduction(float* d_out, float* d_in, int n_threads, unsigned int size)
{
    ...
    while (size > 1)
    {
        ...
        sequential_reduction_kernel<<<n_blocks,n_threads,n_threads*sizeof(float),0>>>(d_out, d_out, size);
        size = n_blocks;
    }
}